...
// MARK: - 初始化一个定时器
public func lgf_RunTimer(_  target: UIViewController!, S: TimeInterval, handler: @escaping (Timer?) -> Void) -> Void {
...
// MARK: - 延时执行
public func lgf_After(S: Double, after: @escaping () -> Void) {
    lgf_After(S: S, queue: DispatchQueue.main, after: after)
}
public func lgf_After(S: Double, queue: DispatchQueue, after: @escaping () -> Void) {
